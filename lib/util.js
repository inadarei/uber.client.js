...
function findQueryMatches(json, key, pattern) {
  ...
  matchedJsonObjects = _.filter(unfilteredJsonObjects, function(element) {
    ...
    var el = String(element.rel);
    return el.match(pattern);
  });

  return matchedJsonObjects;
 ...
 */
function findAllKeyDataEntities(json, key) {
  var prop, subprop;
  ...
  for (prop in json) {
    if (json.hasOwnProperty(prop)) {
      ...
      if (typeof json[prop] === 'object') {
        var next = findAllKeyDataEntities(json[prop], key);
        // concatenate
